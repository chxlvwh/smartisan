<template>
  <div id="iteminfo" class="clear">
    <ul class="imglist">
      <li v-for="img,index in itemIndex.ali_images" @click="changeImg(index)" :class="{ 'active-small-img': imgIndex===index }"><img :src="img" ></li>
    </ul>
    <img :src="itemIndex.ali_images[imgIndex]">
    <div class="info">
      <div class="sku-title">
        <h4>{{itemIndex.title}}</h4>
        <h6>{{itemIndex.sub_title}}</h6>
      ...
      <div class="corlor-num">
        <div class="color">
          <span>颜色</span>
          <ul class="clear">
            <li v-for="color,index in itemIndex.sku_list" :class="{ 'active-color': index==colorSelection }" @click="colorSelect(color, index)"><img :src="'http://img01.smartisanos.cn/' + color.image + '20X20.jpg'"></li>
          </ul>
        </div>
        ...
        <div class="num">
          ...
          <i @click="numPlus" :class="{unclick: num==itemIndex.limit_num}">+</i>
        </div>
      </div>
  ...
  export default {
    ...
    methods: {
      ...
      colorSelect (color, index) {
        // 选择颜色时，切换不同颜色的sku图片
        this.itemId = color.id
        // 给选中的颜色加样式
        this.$store.commit('selectColor', {color, index})
        // 让默认选中左边5张图片中的第一张
        this.imgIndex = 0
        // 初始化num
        this.num = 1
      ...
      numPlus () {
        if (this.num < this.itemIndex.limit_num) {
          this.num++
        }
      },
      ...
      nowBuy () {
      	...
      	let item = Object.assign({},this.itemIndex,{count: this.num})
      	items.push(item)
...
<style scoped>
  ...
  .sku-status button,.sku-status .nowBuy>a{
    width: 145px;
    height: 50px;
    -webkit-box-sizing: border-box;
    -moz-box-sizing: border-box;
