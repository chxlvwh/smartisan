...
//  			let addr = JSON.parse(JSON.stringify(this.info))
        let addr = this.info
  			this.$store.commit('submitAddHandle',addr)
  			this.$store.commit('closeNewAddressHandle')
  		// 修改地址的时候
  		} else {
  			this.$store.commit('changeThisAddrHandle', {aIndex: this.addrIndex, aInfo: this.info})
  			this.$store.commit('closeNewAddressHandle')
  		}
  	},
  	switchMoren () {
