<template>
  <li class="content-goods">
    <router-link target="_blank" class="to-item" :to="{name: 'Item', query: { itemId: item.sku_info[skuIndex].sku_id }}">
    ...
    <ul class="color-select">
      <li v-for="sku,index in item.sku_info" :title="sku.spec_json.show_name" @click="selectSku(index)" :class="{'select-active': isActive(index)}">
        ...
        <div class="cart-btn clear" v-if="btnShow(index)">
          <button>
            <router-link target="_blank" :to="{ name:'Item', query: { itemId: item.sku_info[index].sku_id }}">查看详情</router-link>
          </button>
  ...
  export default {
    ...
    methods: {
      ...
      addCarPanelHandle: function (data) {
        this.$store.commit('addCarPanelData', {data, num: 1})
      },
      btnShow (index) {
        return this.skuIndex === index
      }
